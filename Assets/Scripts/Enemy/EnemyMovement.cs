...
public class EnemyMovement : MonoBehaviour, IHealth
{
    ...
    public void OnPlay(Transform player, Economics economics)
    {
        _player = player;
    ...
    private void Action()
    {
        ...
        {
            if (Vector3.Distance(transform.position, _player.position) <= _distanceFromPlayer)
            {
                Attack();
            }
            else
    ...
    void Attack()
    {
        _personeAtteck.AttackEnemy(_player);
        _time = _timeToStay;
    }
    void Move()
    {
